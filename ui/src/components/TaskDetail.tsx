...
import { X, Copy, Check, Locate } from 'lucide-react'
...
export function TaskDetail({ taskName, edges, onNavigate, onFocus, onClose }: Props) {
  ...
  const statusColor = detail.stale ? 'var(--color-stale)' : 'var(--color-fresh)'
  const statusText = detail.stale ? 'Stale' : 'Up to date'
    ...
    <div className="h-full overflow-y-auto p-4 space-y-3 text-[13px]">
      ...
      <div className="flex justify-between items-start gap-2">
        <div className="min-w-0">
          ...
          <div className="text-[12px] text-[var(--muted-foreground)] mt-1 flex items-center gap-1.5">
            <span className="font-normal">{detail.type}</span>
            <span>·</span>
            ...
            {detail.reason && (
              <>
                <span>·</span>
                <span className="font-normal">{detail.reason}</span>
              </>
            )}
          </div>
        </div>
        ...
        <tbody>
          {detail.deps.length > 0 && (
            <tr className="align-top">
              <td className="text-[var(--muted-foreground)] pr-3 py-[3px] whitespace-nowrap font-normal">deps</td>
              <td className="py-[3px]">
                {detail.deps.map((dep, i) => (
                  <span key={dep}>
                    {i > 0 && ', '}
                    <button onClick={() => onNavigate(dep)} className="text-[var(--color-deps)] hover:underline cursor-pointer">{dep}</button>
                  </span>
                ))}
              </td>
            </tr>
          ...
          {dependents.length > 0 && (
            <tr className="align-top">
              <td className="text-[var(--muted-foreground)] pr-3 py-[3px] whitespace-nowrap font-normal">dependents</td>
              <td className="py-[3px]">
                {dependents.map((dep, i) => (
                  <span key={dep}>
                    {i > 0 && ', '}
                    <button onClick={() => onNavigate(dep)} className="text-[var(--color-dependents)] hover:underline cursor-pointer">{dep}</button>
                  </span>
                ))}
              </td>
            </tr>
          ...
          {detail.inputs.length > 0 && (
            <tr className="align-top">
              <td className="text-[var(--muted-foreground)] pr-3 py-[3px] whitespace-nowrap font-normal">inputs</td>
              <td className="py-[3px] font-mono text-[var(--color-cook-input)]">{detail.inputs.join(', ')}</td>
            </tr>
          )}
          {detail.outputs.length > 0 && (
            <tr className="align-top">
              <td className="text-[var(--muted-foreground)] pr-3 py-[3px] whitespace-nowrap font-normal">outputs</td>
              <td className="py-[3px] font-mono text-[var(--color-output)]">{detail.outputs.join(', ')}</td>
            </tr>
          )}
          {detail.history?.last_succeeded && (
            <tr className="align-top">
              <td className="text-[var(--muted-foreground)] pr-3 py-[3px] whitespace-nowrap font-normal">succeeded</td>
              <td className="py-[3px]">
          ...
          {detail.history?.last_failed && (
            <tr className="align-top">
              <td className="text-[var(--muted-foreground)] pr-3 py-[3px] whitespace-nowrap font-normal">failed</td>
              <td className="py-[3px]">{new Date(detail.history.last_failed).toLocaleString()}</td>
            </tr>
          )}
          {!detail.history && (
            <tr>
              <td className="text-[var(--muted-foreground)] pr-3 py-[3px] font-normal">history</td>
              <td className="py-[3px] text-[var(--muted-foreground)] italic">never run</td>
            </tr>
          )}
          {Object.keys(detail.extra).length > 0 && (
            <tr className="align-top">
              <td className="text-[var(--muted-foreground)] pr-3 py-[3px] whitespace-nowrap font-normal">extra</td>
      ...
      {detail.history?.error && (
        <pre className="text-[12px] bg-[var(--background)] px-3 py-2 rounded-[10px] text-[var(--color-stale)] whitespace-pre-wrap leading-relaxed font-mono">
          {detail.history.error}
        </pre>
      )}
    </div>
